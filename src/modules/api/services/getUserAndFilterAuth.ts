...
export const getUserAndFilterAuth = async (authHeader: string, allowRoles: Role[], ignoreAuthCheck = false) => {
  ...
  const userMetadata = await firebase.firestore().collection('users').doc(decodedToken.uid).get().then(o => o.data()) as User
